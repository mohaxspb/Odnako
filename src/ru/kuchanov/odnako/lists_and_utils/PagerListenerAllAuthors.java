...
public class PagerListenerAllAuthors extends ViewPager.SimpleOnPageChangeListener
{
	...
	public void onPageSelected(int position)
	{
		...
		if (isInRightPager)
		{
			...
			String allAuthorsUrl=CatData.getAllCategoriesMenuLinks(act)[this.currentCategoryPosition];
			
			this.act.getAllCatListsSelectedArtPosition().put(allAuthorsUrl, position);
			
			this.toolbarRight.setTitle(this.allAuthorsUrls.get(position));
			
			
			Intent intentToAllAuthorsFrag = new Intent(allAuthorsUrl + "art_position");
			...
			Intent intentToListFrag = new Intent(this.allAuthors.get(position).getBlog_url()
		...
		else
		{
			...
			if (twoPane)
			{
				...
				pagerRight.setOnPageChangeListener(new PagerListenerArticle(act, curentCategory));
