...
public class ParseBlogsPageBackUp extends AsyncTask<Void, Void, String[][]>
{
	...
	public ParseBlogsPageBackUp(ActionBarActivity act, String categoryToLoad, int pageToLoad, PullToRefreshListView pullToRefreshView, boolean fromBottomPull)
	{
		this.act = act;
	...
	{
		System.out.println("ParceBlogsPage: onPreExecute");
		pref=PreferenceManager.getDefaultSharedPreferences(act);
		//test
		//this.categoryToLoad="neukazano132.odnako.org";
	}

	protected String[][] doInBackground(Void... arg)
	{
		System.out.println("ParseBlogsPage: doInBackground");
		System.out.println("pageToLoad: "+pageToLoad);
		String[][] output = null;
		if (pageToLoad != 0)
		{
			System.out.println("pageToLoad!=0");
		...
		else
		{
			...
			if(this.categoryToLoad.equals("www.odnako.org/authors"))
			{
				System.out.println("this.categoryToLoad.equals('www.odnako.org/authors')");
			...
			else
			{
				...
				String formatedCategory=FormatURLToFileName.format(this.categoryToLoad);
				String formatedPageName=FormatURLToFileName.format(this.categoryToLoad + "/page-1/");
				link = "file:///" + storageDir + "/" + formatedCategory + "/" + "http___" + formatedPageName;
				INITIALpageToLoad=pageToLoad;
				pageToLoad=1;
			}
		...
		try
		{
//			System.out.println("parseBlogsPage link: "+link);
//			link = "http://" + this.categoryToLoad + "/page-" + String.valueOf(this.pageToLoad) + "/";
//			System.out.println("parseBlogsPage link: "+link);
			...
			if(this.categoryToLoad.equals("www.odnako.org/authors"))
			{
				...
				//linkToLocalFile = "file:///" + storageDir + "/" + formatedCategory + "/" + "http___" + formatedPageName;
			...
			ArrayList<String> allCategoryLinks = new ArrayList<String>();
			allCategoryLinks=groupsLinks.get(1);//+groups.get(2)+groups.get(3);
			String[] menuCatLinks = act.getResources().getStringArray(R.array.menu_items_links);
			allCategoryLinks.add(menuCatLinks[2]);
			allCategoryLinks.add(menuCatLinks[3]);
						
			TagNode[] liElemsArr=null;
			
			//System.out.println(allCategotyLinks.size());
			/*for(int i=0; i<allCategotyLinks.size(); i++)
			{
				System.out.println(allCategotyLinks.get(i));
			}*/
			
			for(int i=0; i<allCategoryLinks.size(); i++)
			{
				if(this.categoryToLoad.equals(allCategoryLinks.get(i)))
				{
					liElemsArr= hh.getBlogsInfo();
					i=allCategoryLinks.size();
				}
				else
				{
					if(i==allCategoryLinks.size()-1)
					{
						//liElemsArr = hh.getBlogsInfo();
...
//					ParseBlogsPageNew parse = new ParseBlogsPageNew(act, this.categoryToLoad, 1, this.pullToRefreshView, false);
//					parse.execute();
	...
	void fillData(Integer curPage)
	{
		...
		{
			//MainActivityNew.ALL_ARTS_ARR[curPage].length=HtmlHelper.NUM_OF_ARTS_ON_CUR_PAGE;
		...
		//test checking for empty arts in array
		//System.out.println(MainActivityNew.ALL_ARTS_ARR[curPage].length);
